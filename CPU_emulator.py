...
#cpu = CPU6502()
#cpu.load_program([])  # Clear any existing program

# Test instructions
##test_program = [
##    0xA9, 0x01,  # LDA #$01 (Load 1 into the accumulator)
##    0xA2, 0x02,  # LDX #$02 (Load 2 into the X register)
##    0xA0, 0x03,  # LDY #$03 (Load 3 into the Y register)
##    0x85, 0x10,  # STA $10 (Store the accumulator in memory at address $10)
##    0x86, 0x11,  # STX $11 (Store the X register in memory at address $11)
##    0x84, 0x12,  # STY $12 (Store the Y register in memory at address $12)
##    0xA5, 0x10,  # LDA $10 (Load the value from memory at address $10 into the accumulator)
##    0xA6, 0x11,  # LDX $11 (Load the value from memory at address $11 into the X register)
##    0xA4, 0x12,  # LDY $12 (Load the value from memory at address $12 into the Y register)
##    0x69, 0x05,  # ADC #$05 (Add 5 to the accumulator)
##    0xE8,        # INX (Increment the X register)
##    0xC8,        # INY (Increment the Y register)
##    0x8D, 0x00, 0x02,  # STA $0200 (Store the accumulator in memory at address $0200)
...
##cpu.run()
##
### Print the final state of the CPU registers and memory
##print("Final CPU State:")
##print(f"A: {cpu.A}")
##print(f"X: {cpu.X}")
##print(f"Y: {cpu.Y}")
##print(f"Memory at $10: {cpu.memory[0x10]}")
##print(f"Memory at $11: {cpu.memory[0x11]}")
##print(f"Memory at $12: {cpu.memory[0x12]}")
##print(f"Memory at $0200: {cpu.memory[0x0200]}")
##
